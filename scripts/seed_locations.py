#!/usr/bin/env python3
"""
seed_locations.py — Convert prompt world JSON files to Postgres location records.

Usage:
    python3 scripts/seed_locations.py

Reads all canonical world .json files from data/world/, parses the JSON payload,
...
def parse_location_file(path: Path) -> dict | None:
    """Parse a location JSON file and return a location dict, or None if invalid."""
    try:
...
async def seed(database_url: str) -> None:
    ...
    canonical_files = [p for p in canonical_files if not p.name.startswith("_") and p.name not in {"region.json", "settlement.json", "district.json", "region_zone.json"} and not p.parent.name == "schemas"]

    if not canonical_files:
        print(f"No canonical location .json files found in {CANONICAL_WORLD_DIR}")
        return
