...
|---|---|---|---|---|
| item_radiant_blade_01 | Radiant Blade | weapon | `melee` | On strong success or critical success, may convert minor collateral harm to controlled impact in scene narration. |
| item_wardbreaker_spear_01 | Wardbreaker Spear | weapon | `reach` | Grants narrative justification to contest reinforced barriers or shield lines. |
| item_quicksilver_knife_01 | Quicksilver Knife | weapon | `grappling` | Supports stealth takedown narration when action already succeeds. |
| item_skystring_bow_01 | Skystring Bow | weapon | `ranged` | Enables long-range precision shots in high-wind scenes without contradiction. |
...
| item_monksteel_wraps_01 | Monksteel Handwraps | weapon | `grappling` | Enables meaningful unarmed engagement against armored foes. |
| item_veilcloak_01 | Veilcloak | utility | `disguise_forgery` | Supports identity concealment checks in formal spaces. |
| item_ghoststep_boots_01 | Ghoststep Boots | utility | `grappling` | Supports silent movement framing in infiltration scenes. |
| item_lockwhisper_set_01 | Lockwhisper Picks | utility | `lockpicking_traps` | Supports rapid bypass attempts under time pressure. |
| item_hawkeye_lens_01 | Hawkeye Lens | utility | `ranged` | Improves fictional positioning for long-sight reconnaissance. |
