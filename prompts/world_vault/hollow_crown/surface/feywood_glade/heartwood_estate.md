...
The central space is not a courtyard. It is a clearing of moderate size, kept open but not manicured, with ground that can bear use without being paved. A fire pit of considerable age sits at one side. Oathvine grows up one tree at the clearing's edge — this is House Heartwood's oath-plot, the living record of the family's vows across generations. Oathbloom grows near it in a plot that is plainly tended but not fenced; restraint keeps it safe, not barriers.
...
Oathbloom is tended here in the house's oath-plot. It is the canonical source of ceremonial infusion used for House Heartwood oath observances. It is not harvestable by visitors or traders. Oathvine at the oath-plot edge is a living record of the house's vows.

Threat level 1 reflects the Estate's safety; the surrounding Outer Wild is threat level 3 (inherited from the region-zone). A scene-destination Estate is low-threat; a traveler wandering the Outer Wild around it is not.

`discovered: false` by default; players reach the Estate through canonical narrative paths only.