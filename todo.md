...
- **Brief 21 — Origin vs phase clarification + phase-shift suggestion (2026-05-03, 5.4.0 → 5.4.1).** Addresses the May 2 "emergent forever" failure mode. `prompts/arc-rules.md` reorganized: the old "Origin Types" section is now "Origin vs Phase" — origin is permanent (set at creation), phase is mutable (changes as play develops). New "Phase Change Indicators" section enumerates five structural conditions (council adoption, named patron tasking, formal organization handoff, documented mandate, multi-org engagement) and makes formal-child spawn **required** when any condition is met for an emergent arc. Conversion language tightened from "should usually create" to "cannot be converted in place — origin is permanent; spawn a formal child instead." `ArcEnvelopeStatus` gains `phase_shift_candidate: bool` (additive, default `False`). `_gather_arc_envelope_status` reads `origin_type` and computes `phase_shift_candidate` for emergent arcs at soft cap (but not at hard cap — hard cap is settle territory). `_build_suggestions` orders hard cap > phase shift > soft cap; emergent arcs at soft cap get the more-actionable phase-shift suggestion preempting the generic closure prompt. 5 new tests cover detection (emergent at soft cap → True), discrimination (formal at soft cap → False), early state (under cap → False), and supersession (hard cap supersedes phase shift). `engine.md` untouched. 548 → 553 passing.
- **Brief 20 — Narrator orchestrator (2026-05-03, 5.3.1 → 5.4.0).** `POST /narrator/scene_resolved` composes scene boundary recording, progression validation, advance commit, non-advancement state changes, and time advancement into one transactional flow. Reusable helpers `commit_advance_in_transaction` and `declare_scene_in_transaction` extracted from existing endpoint bodies; the existing `/progression/commit` and `/scene/declare_resolution` endpoints are now thin wrappers around the helpers — zero behavior change. The orchestrator commits any *eligible* proposed advance (the narrator's choice stands); `proposed_evaluation` surfaces `omits_strongest` informationally. Three components (`/scene/declare_resolution`, `/progression/scan`, `/progression/commit`) excluded from the GPT subset since the orchestrator absorbs them; they remain in `openapi.json` for direct testing and admin tooling. Net GPT spec change 30/30 → 28/30 — two slots freed for Briefs 21+ and future combat/crafting orchestrators. New models: `SceneResolvedRequest`, `SceneResolvedResponse`, `CharacterStateChanges`, `WorldStateChanges`. 17 new contract tests cover happy paths, validation failures, transactional rollback, time advancement, location override, envelope status, and the failed-advance recovery path. 531 → 548 passing.
- **Brief 19.5 — JSONB codecs + repository pattern expansion (2026-05-03, 5.3.0 → 5.3.1).** Pure refactor; foundation work for Brief 20's orchestrator. JSON/JSONB codecs registered at pool init in `api/database.py` so asyncpg returns parsed Python objects (not strings) on read; eliminates the `json.loads(x) if isinstance(x, str) else x` defensive pattern from routes/repositories. `StateRepository.get_state_full()` returns a `FullState` dataclass with character + world + log + updated_at. `StateRepository.transaction()` async context manager yields a `TransactionalStateRepository` with optional `lock=True` FOR UPDATE reads — the composition surface Brief 20 builds on. Simple read paths (`scene.py:get_scene_context`, `companion.py:get_companion`, `progression.py:_load_character`) migrated to the repository; transactional paths (`progression_commit`, `declare_scene_resolution`, `spend_ap`, `apply_delta`) kept on inline asyncpg per the incremental-refactor policy. Common error helpers (`session_not_found`, `scene_not_found`, `tag_not_held`, `at_max_tier`) consolidated into `_helpers.py`. Brief renumbering installed: 20 is now the orchestrator (was emergent→formal); 21–24 shift accordingly. Test fakes across the suite updated to return parsed dicts/lists rather than `json.dumps` strings, matching the codec contract. 4 new repository tests; 527 → 531 passing.
- **Brief 19 — Backend scene records (2026-05-03, 5.2.0 → 5.3.0).** New `scene_records` table (Alembic `20260503_0004`) with FK to `game_states`. `POST /scene/declare_resolution` records a scene boundary, validates `arc_progressed_ids` against the session's arcs, computes envelope status (resolved scenes / locations per active arc with soft- and hard-cap flags), and returns suggest-level guidance for cap conditions. `GET /scene/record/{session_id}/{scene_id}` and `GET /scene/records/{session_id}` (cursor-paginated) for orchestrator/admin reads — both excluded from the GPT subset. Brief 18's optional `scene_id` parameter is now active in `/progression/scan` (unknown scene → 422; already-advanced scene → all candidates ineligible + `scene_already_advanced` warning) and `/progression/commit` (unknown → 422; already-advanced → 409 Conflict; success → atomically stamps `tag_advance_committed` inside the same transaction). One-tag-per-scene enforcement now lives at the database. Added `progression` literal to `TypedLogEntry` was already in Brief 18; this brief did not change schema. GPT spec slot trade: `POST /character/{session_id}/spend_ap` excluded (player-UX, never narrator-facing) so `POST /scene/declare_resolution` slots in cleanly. 29 new contract tests (15 scene_records + 7 scene_id wiring + 7 retained progression tests rebuilt around the new validation path). 505 → 534 passing.
...
- [x] **Brief 19 — Backend scene records.** New `scene_records` table; `POST /scene/declare_resolution` records scene boundaries with arc-progression contributions and returns envelope status with soft/hard-cap suggestions. `scene_id` parameter active in `/progression/scan` (already-advanced -> all ineligible + warning) and `/progression/commit` (already-advanced -> 409 Conflict; success atomically stamps `tag_advance_committed`). Admin reads `GET /scene/record/...` and `GET /scene/records/...` live in `openapi.json` only. Slot trade: `spend_ap` out of GPT subset, `declare_resolution` in. Landed as 5.3.0.
- [x] **Brief 19.5 — JSONB codecs + repository pattern expansion.** Pure-refactor transitional step before Brief 20. Pool init registers JSON/JSONB codecs so JSONB columns return parsed objects (eliminates the `json.loads(x) if isinstance(x, str) else x` defensive pattern from routes/repositories). `StateRepository.get_state_full()` returns a `FullState` dataclass; `StateRepository.transaction()` async context yields a `TransactionalStateRepository` with optional FOR UPDATE locks — the surface Brief 20's orchestrator composes through. Migrated simple read paths (`scene.py`, `companion.py:get_companion`, `progression.py:_load_character`) to the repository; transactional paths kept as-is for incremental refactor. Common 404/422 helpers (`session_not_found`, `scene_not_found`, `tag_not_held`, `at_max_tier`) consolidated into `_helpers.py`. Brief renumbering: 20 is now the orchestrator (was emergent→formal); 21 is emergent→formal; 22 is prompt audit; 23 is registry/catalog unification; 24 is log management. Landed as 5.3.1.
- [x] **Brief 20 — Narrator orchestrator.** `POST /narrator/scene_resolved` composes scene boundary recording, progression validation, advance commit, non-advancement state changes, and time advancement into one transactional flow. Eligible-but-omits-strongest advances still commit — narrator's choice stands; the orchestrator surfaces the omission informationally. Three absorbed components (`/scene/declare_resolution`, `/progression/scan`, `/progression/commit`) excluded from the GPT subset; remain in `openapi.json` for admin/testing. GPT spec at 28/30, two slots freed. Landed as 5.4.0.
- [x] **Brief 21 — Origin vs phase clarification.** `arc-rules.md` reorganized into Origin vs Phase + Phase Change Indicators sections; conversion language tightened from "should usually" to required workflow. `ArcEnvelopeStatus` gains `phase_shift_candidate`; `_build_suggestions` emits phase-shift guidance for emergent arcs at soft cap (preempts the generic soft-cap closure suggestion). Backend surfaces; narrator confirms structural conditions before spawning formal child. Landed as 5.4.1.

### Phase 3 — Prompt collapse + catalog unification (Briefs 22–24)
...
- [x] **Brief 22 — Prompt content audit and shrink.** Four files touched: `progression-rules.md` (-58 lines), `scene-structure.md` (-7 lines), `arc-rules.md` (-105 lines), `engine.md` (-208 bytes). Procedural content the backend now serves authoritatively retired; conceptual/judgment content preserved. The 9 mixed-content prompts (combat, mechanics-tables, items, economy, magic, difficulty, character-creation, calendar, world-rules) deferred to a follow-on **Brief 22.5** since they require per-section judgment about which content the backend has actually displaced. The 6 narrative prompts (world, geography, history, npcs, groups, design-notes) hold prose for narration and are not affected. Landed as 5.4.2.
- [x] **Brief 23 — Registry / catalog unification.** `data/tags/applications.json`, `data/tags/knowledge_groups.json`, and the three `_template_*.json` files moved to `data/catalog/registries/` via `git mv`; `data/tags/` directory removed. Six `api/game_data.py` references and four `scripts/validate_data_files.py` references repointed; `docs/items-schema.md` and the May 2 architecture review/system audit updated. Pure file relocation: no schema, API surface, or behavior change. Other parallels (`data/economy/`, `data/magic/`, `data/companions/`, `data/characters/`, `data/npcs/`) deferred — those are not duplicate namespaces but distinct concerns. Landed as 5.4.3.
- [x] **Brief 24 — Log management strategy.** `log_limit` query parameter on `GET /state/{session_id}` returns only the most recent N log entries (1–10000); `log_total_entries` field added to `GameStateResponse`. Existing manual-compression path (`/state/{session_id}/delta` with `type: "compression"`) preserved as the only compression interface — no new compression endpoint, no auto-prune, no retention windows. `docs/operations/log-management.md` captures the strategy. Patch bump (5.4.3 → 5.4.4); pure additive. Backend Authority Arc structurally complete.
...
### Catalog stabilization follow-ups (resolved by Brief 23)

The Brief 23 unification (2026-05-03, 5.4.3) consolidated the `data/tags/` ↔ `data/catalog/registries/` parallels. `data/tags/applications.json` and `data/tags/knowledge_groups.json` moved into `data/catalog/registries/` (joining `magic_fields.json` from the 2026-05-01 consolidation); `data/tags/` directory removed.

- [x] `data/tags/applications.json` and `data/tags/knowledge_groups.json` consolidated into `data/catalog/registries/` (Brief 23).
- [ ] `data/economy/` vs `data/catalog/economy/` — distinct concerns, not duplicate namespaces; not a Brief 23 target.
- [ ] `data/magic/`, `data/companions/`, `data/characters/`, `data/npcs/` — distinct concerns, not duplicate namespaces; not a Brief 23 target.
