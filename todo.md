...
- [ ] **Brief 20 — Narrator orchestrator.** `POST /narrator/scene_resolved` composes `/scene/declare_resolution`, `/progression/scan`, `/progression/commit` (and possibly `/state/{id}/delta`) into a single transactional flow built on `StateRepository.transaction()`. Hybrid input (structured fields + free-text scene_summary). Suggest-level enforcement of arc envelopes. Frees GPT spec slots by removing the now-internal components from the GPT subset.
