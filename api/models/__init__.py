"""
api.models — Pydantic v2 models for core game entities.

Designed for Mystic Weave 5.0.0: d100 roll-under, domain scores,
nested knowledge/magic records (v5 shape — groups contain applications,
fields contain spells), fungible advancement, four-layer character system
built from ancestry, culture, background, and focus.

Character-related models live in `api.models.character` (Brief 13). All
character types are re-exported from this package root so the historical
`from api.models import CharacterModel` import sites keep working.
"""
